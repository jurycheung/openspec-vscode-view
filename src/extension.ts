 ...
 *
 * activate 只负责组装：树/面板的创建、rebuild 与 refreshAll 的编排。
 * 配置流、对账层、命令注册分别由本文件的三个模块级工厂承担。
 */

import * as os from 'node:os';
import * as vscode from 'vscode';
import { detectOpenspecModels, detectOpenspecRoots, hasOpenspecLayout, type OpenSpecModel } from './openspec';
import { ChangesTreeProvider, type TreeNode } from './tree';
...
// ---------------------------------------------------------------------------
// 扫描路径配置流（保存在 ~/.sef/config.json）
// ---------------------------------------------------------------------------

/** 首次创建配置文件时，预置工作区与主目录绝对路径作为示例（用户可自由删改）。返回配置文件路径 */
function seedConfigFile(): string {
  const seeds: string[] = [];
...
/** 创建扫描路径配置 QuickPick 入口（添加 / 编辑配置文件 / 点击已配置路径移除） */
function createScanPathConfig(rebuild: () => void): () => Promise<void> {
  /** 构建 QuickPick 条目：添加 / 编辑配置文件 / 已配置路径（点击移除） */
  const buildScanPathItems = (): Array<
    vscode.QuickPickItem & { action?: 'add' | 'edit'; removePath?: string }
  > => {
  ...
  return async (): Promise<void> => {
    ...
    if (picked.action === 'edit') {
      const configFile = seedConfigFile();
...
// ---------------------------------------------------------------------------
// 刷新兜底对账层：个别 watcher 后端不上报「目录删除」事件，多层对账保证最终一致
// ---------------------------------------------------------------------------
 ...
 * 3) 树视图重新可见时对账
 * 4) 工作区结构变化（打开/切换文件夹）→ 全量重探；扩展配置变化 → 对账 + 刷新
 */
function setupReconciliation(context: vscode.ExtensionContext, deps: ReconcileDeps): void {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeWorkspaceFolders(() => deps.rebuild()),
    ...
    deps.treeView.onDidChangeVisibility((e) => {
      if (e.visible) {
        void deps.refreshAll();
      }
    }),
...
interface CommandDeps {
  models: () => OpenSpecModel[];
  coordinator: ProcessViewCoordinator;
  refreshAll: (reason?: string) => Promise<void>;
  treeView: vscode.TreeView<TreeNode>;
    ...
    vscode.commands.registerCommand(
      ...
      () =>
        ...
        vscode.window.withProgress(
          ...
          () => deps.refreshAll('manual')
        )
    ),

    vscode.commands.registerCommand('openspec-vscode-view.configure', () => deps.configure()),
    ...
    vscode.commands.registerCommand(
      ...
      async (modelIndexOrName?: number | string, maybeName?: string) => {
        if (typeof modelIndexOrName === 'number' && typeof maybeName === 'string') {
          deps.coordinator.show(modelIndexOrName, maybeName);
          return;
        }
        if (typeof modelIndexOrName === 'string' && deps.models().length > 0) {
          deps.coordinator.show(0, modelIndexOrName);
          return;
        }
        // 树节点点击（无参数调用）：从当前选中项解析上下文
        const node = deps.treeView.selection[0];
        if (node && node.kind === 'change') {
          deps.coordinator.show(node.modelIndex, node.change.name);
          return;
        }
        if (node && node.kind === 'artifact') {
          deps.coordinator.show(node.node.modelIndex, node.node.change.name);
          return;
        }
        await deps.coordinator.showPick();
      }
    ),
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  setupReconciliation(context, { rebuild, refreshAll, treeView });
  registerCommands(context, {
    models: () => models,
    coordinator,
    refreshAll,
    treeView,
    configure: createScanPathConfig(rebuild),
  });
